...
(async () => {
  const approvedSlangs = await GZData.getApprovedSlangs();
  ...
  ALPHABET.forEach(l => {
    ...
    // ... rest of the forEach stays exactly the same
  });

  await renderSlangs(null); // initial render
})();
...
ALPHABET.forEach((l) => {
  const btn = document.createElement("button");
  btn.className =
    "letter-btn" + (lettersWithSlangs.has(l) ? " has-slangs" : "");
  btn.textContent = l;
  btn.addEventListener("click", async () => {
    if (activeLetter === l) {
      activeLetter = null;
      btn.classList.remove("active");
      await renderSlangs(null);
    } else {
      document
...
// Render
async function renderSlangs(letter) {
  const all = await GZData.getApprovedSlangs();
  const list = letter
    ? all.filter((s) => s.letter?.toUpperCase() === letter)
    : all;
...
function createSlangCard(s) {
  ...
  card
    ...
    .addEventListener("click", () => handleVote(s.id, "like"));
  card
    .querySelector(".dislike-btn")
    .addEventListener("click", () => handleVote(s.id, "dislike"));
  ...
  if (session) {
    card.querySelector(`.comment-submit`).addEventListener("click", () => {
      ...
      if (found) {
        ...
        document.getElementById(`clist-${s.id}`).innerHTML = renderCommentList(
          found.comments,
        );
        card.querySelector(".comments-toggle").textContent =
...
function renderCommentList(comments) {
  if (!comments.length)
    return '<p style="font-size:.75rem;color:var(--text2)">No comments yet.</p>';
  return comments
    .map(
      (c) => `
...
function handleVote(id, type) {
  ...
  if (card) {
    card.querySelector(".like-btn").className =
      `like-btn${liked[id] ? " active" : ""}`;
    card.querySelector(".like-btn span").textContent = s.likes;
    card.querySelector(".dislike-btn").className =
      `dislike-btn${disliked[id] ? " active" : ""}`;
